###############################################################################
# 15_rcpp_bridge.R
# Rcpp acceleration bridge for the decoupled HierCPD code path.
#
# Decoupled atom semantics:
#   - DP atoms contain (gamma1, gamma2, shape_beta, shape_gamma)
#   - series/state intercepts gamma^*_{j,k} live in params$gamma[[j]][k]
#
# [PERF] Module-level D-cube cache (set_precomp_datx / G_DATX in the C++):
#   RcppArmadillo copies an arma::cube argument on EVERY call (SEXP->cube).
#   The ESS hot path (log_lik_series_cpp called J x slice-iters per sweep)
#   therefore used to re-marshal the entire (M+1)x(M+1)xT cube thousands of
#   times per sweep. The C++ kernels no longer take the cube as an argument;
#   instead it is pushed ONCE via set_precomp_datx() and read from G_DATX.
#   .hiercpd_ensure_datx(precomp) performs that push lazily, re-pushing only
#   when the cube identity (dims + corners) changes.
#
# Behavior:
#   - If Rcpp/RcppArmadillo are available, hiercpd_core.cpp compiles, AND the
#     decoupled-atom + cube-cache compatibility check passes, the hottest
#     continuous-model inner loops are overridden by Rcpp-backed equivalents.
#   - Otherwise the pure-R reference implementation remains active.
###############################################################################

.hiercpd_rcpp_enabled <- FALSE

# No-op fallback so callers (06/07) can guard with a single exists() check and
# invoke it unconditionally in pure-R mode. Overwritten by the real version
...
.hiercpd_try_compile_cpp <- function() {
  ...
  # --- Decoupling + cube-cache compatibility check ---
  # Verifies (a) the compiled C++ understands shape_beta/shape_gamma atoms and
  # (b) it exposes the cube-cache API set_precomp_datx() together with the
  # no-cube eval_shape_at_times_cpp signature. An older C++ (kappa_star atoms,
  # or cube passed per call) errors/mismatches here and triggers the R fallback.
  if (ok) {
    compat_ok <- tryCatch({
      ...
      # Decoupled formula with shape_gamma=0.5 and D=0 (=> H=0):
      #   linear_coef = delta1*shape_gamma + shape_beta*(delta1+delta2)/2 = 1.5
      #   res[2] = linear_coef * x[2] = 1.5 * 0.5 = 0.75
      abs(res[2] - 0.75) < 0.01
