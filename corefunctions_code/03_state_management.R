...
init_state <- function(J, T_len, C, K_init, m_min, fixed_cluster_sizes = FALSE) {
  ...
  if (fixed_cluster_sizes) {
    # 균등 블록 할당: 각 클러스터에 J/C개 (나머지는 순차 배분)
  ...
  # Upper-level states: evenly spaced changepoints
  ...
  # Lower-level states: initially same as upper
...
ensure_atoms_consistency <- function(state, params, model) {
  ...
  for (cc in 1:C) {
    ...
    # [γ*(c,j)] γ*는 시계열 단위 스칼라이므로 relabel/resize 불필요
    j_in_c <- which(state$cluster == cc)
...
compute_free_obs_upper <- function(k, c_idx, state, m_min, virtual_offset = 0L) {
  ...
  # Left bound: tau_k + m_min
  # [VIRTUAL AUGMENT] k==1이면 effective tau_1 = tau[1] - virtual_offset
  #   → left = (tau[1] - virtual_offset) + m_min
  #   virtual_offset == m_min이면 left = tau[1] = 1 (첫 CP가 t=1부터 허용)
  tau_k_eff <- if (k == 1L && virtual_offset > 0L) tau[k] - virtual_offset else tau[k]
  left <- tau_k_eff + m_min
  left <- max(left, tau[k] + 1L)  # 첫 상태에 최소 1개 실제 관측 보장 (R의 1:0=c(1,0) 방지)
  if (is.na(left)) return(integer(0))
  
  # Right bound: tau_{k+2} - m_min (or T - m_min + 1 if k+1 is last state)
...
compute_free_obs_lower <- function(j, k, state, m_min, virtual_offset = 0L) {
  ...
  left <- tau_k_eff + m_min
  left <- max(left, tau_L[k] + 1L)  # 첫 상태에 최소 1개 실제 관측 보장
  if (is.na(left)) return(integer(0))
  
  # Right bound: 다음 LOWER 경계 tau^L_{j,k+2} - m_min (series 자신의 경계).
  #   [BUG FIX] 이전에는 upper 경계 tau^U_{c,k+2} 를 썼는데, lower 경계가 upper 와
  #   다를 때(계열별 changepoint 위치 상이) tau^L_{j,k+1} 가 tau^L_{j,k+2} 를 침범해
  #   [tau^L_{j,k+1}, tau^L_{j,k+2}) 세그먼트가 m_min 미만이 되어 C2 위반 →
  #   enforce_all_invariants 의 강제병합이 MCMC 틀 밖에서 발동했다. left bound 가
  #   tau_L[k] 인 것과 대칭으로 right 도 tau_L[k+2] 를 써야 한다.
...
get_effective_variance <- function(j, t, state, params) {
  # [v_j 스칼라화] regime 인덱싱 제거 — v_j는 시리즈 전체 공유
  v_j <- params$v[[j]]
...
get_effective_variance_all <- function(j, state, params) {
  T_len <- ncol(state$S_lower)
  # [v_j 스칼라화] regime-invariant: 시리즈 전체에 동일한 기저 분산
  v_j <- params$v[[j]]
...
get_state_variance <- function(j, k, params) {
  # [v_j 스칼라화] k 인자는 하위 호환을 위해 유지하되 사용하지 않음
  v_j <- params$v[[j]]
...
# Call after any state modification (IA, LA) to guarantee:
#   I1. S_upper and S_lower are non-decreasing (C1)
#   I2. Every segment has >= m_min observations (C2)
#   I3. Labels are consecutive 1,2,...,K
#   I4. K_upper == K_lower for all series in each cluster
###############################################################################
