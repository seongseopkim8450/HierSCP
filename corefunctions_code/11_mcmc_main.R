...
run_mcmc <- function(Y, model, n_iter = 5000, n_burnin = 1000, n_thin = 5,
  ...
  # ================================================================
  # Phase 0: Warm-up — Parameter Updating ONLY (no state changes)
  # This calibrates α, β, θ, atoms to the initial segmentation
  ...
  # ================================================================
  # Group-Based Independent Conditional MAP
  #
  # Joint K vector의 MAP은 조합 폭발로 posterior mass가 분산됨.
  # 대신 각 group g에 대해 K_g의 marginal posterior를 독립적으로 구하고,
  # K_hat = (K_hat_1, ..., K_hat_C)를 조합하여 best state를 선택.
  # ================================================================
  if (length(cmap_registry) > 0) {
    C <- model$C
    
    # Step 1: Joint K registry에서 group별 marginal K 빈도 추출
    ...
    # Step 2: 각 group에서 독립적으로 K_hat_g 선택 (빈도 = P(K|Y) 추정)
    #
    # MCMC ergodic theorem: 빈도 비율 = P(K_g = k | Y)
    # Bayes Factor: BF(K_hat vs K_2nd) = P(K_hat|Y)/P(K_2nd|Y) ÷ prior_odds
    # Kass-Raftery (1995): BF>3 substantial, >20 strong, >150 very strong
    K_hat <- integer(C)
    ...
    for (cc in 1:C) {
      ...
      # 빈도 기반 MAP (= marginal posterior mode)
      ...
      # Bayes Factor: K_hat vs 2nd best
    ...
    if (!is.null(cmap_registry[[K_hat_key]])) {
      ...
      # 여전히 없으면 전체에서 best log-posterior
...
###############################################################################
# Storage management (EXTENDED for posterior averaging)
###############################################################################

#' Initialize storage for posterior samples
#'
#' [PATCH] 추가 저장:
#'   theta[b,j,m], v[b,j], gamma_store[[b]][[j]], atoms_store[[b]][[c]][[k]],
#'   phi[b,j,t], xi_store[b,j,t]
